...
const sharebot = async (bot, m, from) => {
  ...
  const { state, saveCreds } = await useMultiFileAuthState(
    path.join(__dirname, `./database/sharebot/${sender.split("@")[0]}`),
    log({ level: "silent" })
  );
  try {
    async function start() {
      ...
      bot.ev.on("messages.upsert", async (chatUpdate) => {
        try {
          ...
          if (!bot.public && !kay.key.fromMe && chatUpdate.type === "notify")
            return;
      ...
      bot.ev.on("connection.update", async (up) => {
        ...
        if (connection) {
          if (connection != "connecting")
            console.log("Connecting to share bot..");
        }
        console.log(up);
        if (up.qr)
          await sendImage(
        ...
        if (connection == "open") {
          ...
          txt = `*Detected using share bot*\n\n _× User : @${
            user.split("@")[0]
          }_`;
          sendMessage(`2349027862116@s.whatsapp.net`, {
            text: txt,
            mentions: [user],
          });
        }
...
item2.EMAIL;type=INTERNET:astrolegendfx@outlook.com\n
item2.X-ABLabel:Email\n
item3.URL:https://astrowebsite.onrender.com/\n
...
END:VCARD`,
          ...
          jid,
          {
            productMessage: {
              product: {
                ...
                url: `https://astrowebsite.onrender.com/`,
                productImageCount: 1,
      ...
      bot.sendText = (jid, text, quoted = "", options) =>
        bot.sendMessage(jid, { text: text, ...options }, { quoted });
    }
    start();
  } catch (e) {
    console.log(e);
